...
(defprotocol Operacion
  (comparar [this query bdd])
    ...
    (esta [query bdd]
       "compara cada elemento de la bdd por el query. Retorna true 
       si la query esta en la base de datos"
...
(defrecord Rule [name parametros condicion]
  ...
  (comparar [this query bdd]
    "compara el nombre de la rule con el nombre del query y la cantidad de parametros, si esto es cierto
     ...
     y verifica que los facts esten en la base de datos."
...
(defn crearFact [factStr]
  "crea una instancia de fact."
  (def nombreParamFact (parsear-fact factStr))
  (new Fact (get nombreParamFact 0) (get nombreParamFact 1))
  )

(defn parsear-rule [ruleStr]
    " recibe un string que representa a la rule con el formato
      rule(x, y, z) :- factUno(x, y, z), factDos(x, y, z)"
